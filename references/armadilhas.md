...
| Problema | Causa | Solução |
|---|---|---|
| A maior parte do mês some | A API esconde tarefas arquivadas por padrão, mesmo com `include_closed=true`, e a NLR arquiva a tarefa assim que vira "reembolsado pelo cliente" | Buscar `archived=false` **e** `archived=true` e juntar |
...
| EXIF vazio nas imagens | Screenshots e downloads não preservam metadados | Ler visualmente com a ferramenta `Read`: o comprovante BTG mostra valor, data e favorecido |

## Identificação de comprovantes

| Problema | Causa | Solução |
|---|---|---|
| Duas demandas com o mesmo valor | Nada nos custom fields diferencia | Usar o horário da sessão de pagamento no comprovante BTG |
| Não dá para saber de qual pedido é o recibo |, | O campo "Mensagem" do comprovante BTG traz um protocolo que corresponde ao "pedido nº" do recibo ONR. É o cruzamento mais confiável quando existe |
| Descartei o comprovante e entreguei o brink | Decisão tomada pelo nome do arquivo | Julho/2026: `ONR - larga dos bois.pdf` é comprovante Pix de R$ 39,16; `Pagametnto CCIR pix.pdf` é PagTesouro "Aguardando realização do pagamento". Abrir e olhar, sempre |
